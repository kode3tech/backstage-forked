...
import { createPolyFetchCatalogEntityAction } from './fetch';
...
describe('catalog:fetch', () => {
  ...
  const action = createPolyFetchCatalogEntityAction({
  ...
  describe('fetch single entity', () => {
    it('should return entity from catalog', async () => {
      ...
      expect(mockContext.output).toHaveBeenCalledWith('results', [{
        entity: {
          metadata: {
            namespace: 'default',
            name: 'test',
    ...
    it('should use defaultKind and defaultNamespace if provided', async () => {
      ...
      expect(mockContext.output).toHaveBeenCalledWith('results', [{'entity': entity}]);
    });
  });
